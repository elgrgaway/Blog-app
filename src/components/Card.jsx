...
function Card(props) {
  ...
  return (
    <div className=" border-[#E8E8EA] rounded-xl border-solid border-2   p-4 ">
      ...
      <span className="text-[#4B6BFB] text-sm font-medium bg-[#F6F8FF] py-1 px-2.5 rounded-md">
        {category}
      </span>
      <Link to={name}>
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="font-medium text-[#97989F]">{name}</span>
        <span className="font-medium text-[#97989F]">{publishedAt}</span>
      </div>
    </div>
  );
}
